...
const smallSizesArr = [];
const possibleToDeleteArr = [];

function getTotal(node, total = 0) {
  if (!node.children.length) {
    return;
  }
  ...
  if (node.size <= 100000) {
    smallSizesArr.push(node.size);
  }

  if (head.size - node.size <= 40000000) {
    possibleToDeleteArr.push(node.size);
  }
}

getTotal(head);

console.log(possibleToDeleteArr.sort((a, b) => (a > b ? 1 : -1)));
